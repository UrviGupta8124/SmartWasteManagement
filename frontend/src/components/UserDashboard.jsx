...
export default function UserDashboard() {
  ...
  // ── NEW: real waste breakdown from WasteLog ──
  ...
  // Fetch user profile + weekly activity (existing)
  ...
  // ── NEW: fetch real waste counts from WasteLog ──
  const fetchWasteStats = async () => {
    try {
      const res = await axios.get('/dashboard/waste-stats/user');
      setWasteStats(res.data);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    // ── NEW: Socket.io — live update when Python classifies a new item ──
    const socket = io(SOCKET_URL);
    socket.on('new_waste_log', (log) => {
      // Only refresh if this log belongs to current user
      if (log.userId === user?.id || log.userId === user?._id) {
        fetchWasteStats();
        fetchDashboard();
      }
    });
  ...
  const handleRedeem = async (reward) => {
    ...
    if (window.confirm(`Are you sure you want to redeem "${reward.title}" for ${reward.pointsCost} pts?`)) {
      try {
        ...
        setData(prev => ({ ...prev, user: { ...prev.user, points: res.data.userPoints } }));
      } catch (err) {
  ...
  const handleComplaintSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('/complaint', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
      showToast('Complaint sent to municipality successfully! 📝');
  ...
  // ── Use real wasteStats if loaded, fallback to existing data.wasteBreakdown ──
  const wasteBreakdownData = wasteStats
    ? wasteStats.chartData
    : data.wasteBreakdown;

  const totalItems = wasteStats
    ? wasteStats.totalScans
    : wasteBreakdownData.reduce((a, b) => a + b.value, 0);
      ...
      <div className="ud-dashboard-wrapper">
        ...
        <nav className="ud-navbar">
          ...
          <div className="ud-nav-right">
            ...
            <button onClick={logout} className="ud-btn ud-btn-secondary ud-btn-logout" title="Sign Out">
              Sign Out
            </button>
          </div>
        </nav>
              ...
              <div className="ud-charts-row">
                <div className="ud-panel">
                  <div className="ud-panel-header">
                    ...
                    {/* ── NEW: show live badge if real data loaded ── */}
                    {wasteStats && (
                ...
                <div className="ud-panel">
                  <div className="ud-panel-header">
                    <h3 className="ud-panel-title"><Activity size={18} color="#94a3b8" /> Waste Items Today</h3>
                    {wasteStats && (
                      <span style={{ fontSize: '11px', color: '#10b981', fontWeight: 600, background: '#d1fae5', padding: '2px 8px', borderRadius: '99px' }}>
                        ● LIVE
                      </span>
                  ...
                  <div style={{ height: '270px' }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={wasteBreakdownData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }} barSize={40}>
                        ...
                        />
                        <Bar dataKey="value" name="Items classified" radius={[6, 6, 0, 0]}>
                          {wasteBreakdownData.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={entry.fill || COLORS[index % COLORS.length]} />
                          ))}
                        </Bar>
